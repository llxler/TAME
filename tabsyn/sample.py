...
from tabsyn.diffusion_utils_gen import sample
...
def main(args):
    ...
    x_next = sample(model.denoise_fn_D, num_samples, sample_dim, mean, 
                    info, num_inverse, cat_inverse, dataname, device=device)
    # x_next = sample(model.denoise_fn_D, num_samples, sample_dim)
    x_next = x_next * 2 + mean.to(device)

    # syn_data = x_next.float().cpu().numpy() # ori
    ...
    # Evaluation
    eval_all("", save_path, dataname=dataname)
